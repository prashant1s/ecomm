...
export default function Header() {
  ...
  return (
    <>
      <header
        className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? "bg-white shadow-md py-4" : "bg-transparent py-0.5"}`}
      >
        <div className="container mx-auto px-6 flex justify-between items-center">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2">
            {/* Increased size to 64x64, removed the text span entirely */}
            <Image
              src="/icon512x512.png"
              alt="Logo"
              width={50}
          ...
          <nav className="hidden md:flex gap-8">
            {navLinks.map((link) => (
              <Link
                key={link}
                href="#"
                className={`font-medium hover:text-blue-600 transition-colors ${isScrolled ? "text-gray-800" : "text-white drop-shadow-sm"}`}
              >
                {link}
              </Link>
            ))}
          ...
          {/* Icons */}
          <div
            className={`flex items-center gap-6 ${isScrolled ? "text-black" : "text-white"}`}
          >
                  ...
                  items.map((item) => (
                    <div key={item._id} className="flex gap-4 border-b pb-4">
                      ...
                      <button
                        ...
                        className="text-red-500 text-sm h-fit"
                      >
                        Remove
                      </button>
                    </div>
